...
def hello_world():
    return render_template('isb_home.html')

@app.route('/isb_home', methods=['GET'])
def Home():
    return render_template('isb_home.html')

@app.route('/Achievement_isb', methods=['GET'])
def Achievement():
    return render_template('Achievement_isb.html')

@app.route('/About_us_isb', methods=['GET'])
def About_us():
    return render_template('About_us_isb.html')
...
def insertintotable():
    if request.method == 'POST':
        ...
        im_1_path="./Flask_input/"+image.filename
        image.save(im_1_path)
        A = overlay_ocr_text(im_1_path)
        A.save('static/cropped_image.png')

        return render_template('isb_home.html', filename='cropped_image.png')
